//! Generates Tauri context, command permissions, and platform resources.

fn main() -> Result<(), Box<dyn std::error::Error>> {
    tauri_build::try_build(tauri_build::Attributes::new().app_manifest(
        tauri_build::AppManifest::new().commands(&[
            "query_function_flows",
            "activate_catalog_project",
            "cancel_model_probe",
            "list_recent_projects",
            "cancel_deep_map",
            "compile_task_lens",
            "configure_model_provider_v2",
            "delete_model_provider_credential_v2",
            "confirm_project_command_allowlist",
            "control_agent_task_run",
            "control_agent_session",
            "control_agent_session_v2",
            "control_agent_session_queue",
            "create_agent_goal",
            "discover_provider_models_v2",
            "open_project",
            "pause_deep_map",
            "query_deep_map",
            "query_deep_map_entries",
            "query_deep_map_entry_detail",
            "query_deep_map_run_dashboard",
            "query_deep_map_run_modules",
            "query_deep_map_module_steps",
            "query_deep_map_atlas_impact",
            "query_deep_map_runs",
            "query_project_catalog",
            "query_index_activity",
            "query_index_run_inspection",
            "query_index_run_files",
            "control_index_run",
            "query_index_overview",
            "query_agent_activity",
            "query_agent_ask_research_turns",
            "query_agent_ask_research_detail",
            "query_agent_ask_research_sources",
            "query_agent_ask_research_source_preview",
            "query_agent_diagram_artifacts",
            "query_agent_diagram_artifact",
            "export_agent_diagram",
            "query_agent_work_trace_turns",
            "query_agent_work_trace_detail",
            "query_agent_work_trace_detail_v2",
            "query_agent_work_trace_projection",
            "query_agent_work_trace_sources",
            "query_agent_work_trace_sources_v2",
            "query_agent_work_trace_source_preview",
            "query_agent_session",
            "query_agent_session_v2",
            "query_agent_session_v3",
            "query_agent_sessions",
            "query_agent_slash_commands",
            "query_agent_goal",
            "query_agent_task_recovery",
            "query_module_card_freshness",
            "query_module_card_detail",
            "query_module_card_evidence",
            "query_module_dependency_graph",
            "query_module_runtime_flow",
            "query_module_runtime_map",
            "query_module_tree",
            "query_project_map_search",
            "query_repository_tree",
            "query_task_lens_task",
            "query_task_lens_tasks",
            "query_project_status",
            "query_project_settings",
            "query_health",
            "query_settings_v2",
            "query_settings_recovery",
            "recover_invalid_model_profiles",
            "query_ui_preferences",
            "rebuild_project_index",
            "resume_deep_map",
            "restore_last_project",
            "remove_project",
            "remove_catalog_project",
            "probe_model_role_v2",
            "set_model_provider_credential_v2",
            "set_model_provider_enabled_v2",
            "revise_agent_goal",
            "start_deep_map",
            "submit_agent_message",
            "submit_agent_message_v2",
            "submit_agent_message_v3",
            "submit_agent_message_v4",
            "continue_agent_research",
            "update_agent_workspace_layout",
        ]),
    ))?;
    Ok(())
}
