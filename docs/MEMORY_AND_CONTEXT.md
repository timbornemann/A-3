...
Der nach [ADR-0102](adrs/0102-originalgebundene-operationshinweise.md) getestete
Operationszusatz ist nach dem [negativen Modellvergleich](plans/10-SOURCE_OPERATIONS_VALIDATION.md)
aus dem aktuellen Laufzeitpfad entfernt. Trotz tatsächlich gelieferter Aufruf- und
Return-Schritte behielten die kurzen Modellinterpretationen die entscheidenden
Informationen nicht zuverlässig. SourceReview V2 arbeitet wieder ohne diesen
Zusatz; Produktstandard bleibt `joint`. Vorhandene explizite Flow-Reads und ihre
unabhängigen Quellen-/Freshness-Grenzen bleiben erhalten. Die native Matrix injiziert
